...
export class Cons{
  _is_Cons = true;
  constructor(public car : Thing,
              public cdr : Thing){}
  public toString():string{
    return safePrintThing(this);
  }
  get(i:number):Thing|undefined {
    if (i < 0) {return undefined;} //array behaviour
    if (i == 0) {return this.car;}
    if (isCons(this.cdr)) { return this.cdr.get(i-1);}
    return undefined; //array behaviour
  }
  cr(pattern:number):Thing | undefined{
    if (pattern <= 1){ return this;}
    const r = (pattern & 1)? this.car:this.cdr;
    pattern = Math.floor(pattern/2);
    if (isCons(r)){return r.cr(pattern);}
    if (pattern <= 1){ return r;}
    return undefined;
  }
}
...
export function prettyPrintThing(c:Thing,seen:Set<Cons>|null=null,num:Map<Cons,number>|null=null,show:boolean=false):string{
  if (seen === null){seen = new Set<Cons>();}
  if (num === null){num = new Map<Cons,number>();}
  if (c === null){
    return "()"
...
export function safePrintThing(c:Thing):string{
  ...
  function see(c:Thing,seen:Set<Cons>,num:Map<Cons,number>,n={v:0}){
    if (isCons(c)){
      if (seen.has(c)){
        if (!num.has(c)){
          num.set(c,++n.v);
...
function parse_thing(toks: RegExpMatchArray,i:number,defs:Map<string,ParseVal>): ParseRes {
  ...
  // quoted | ATOM | cons
  const res = toks[i] === "'" ? quote(parse_thing(toks,i+1,defs)) :
    (s_atom.test(toks[i]) ? {i:i+1,v:toks[i]}  :
